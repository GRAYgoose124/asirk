...
class Factoid(Plugin):
    def __init__(self, protocol):
        ...
        # TODO: mysql DB
        self.dbfile = os.path.join(self.protocol.config["plugin_data_path"], "factDB")
        try:
    ...
    def msg_hook(self, event):
        ...
        if (
            sender == "NICKSERV"
            or event.cmd != "PRIVMSG"
            or message[0] == self.protocol.bot.command_symbol
            or message[0] == ":"
        ):
            return
    ...
    def factoid(self, event):
        ...
        if cmd == "new":
            ...
            if data[0] == "" and data[-1] == "":
                ...
                m = "'{} <{}> {}' is now recorded.".format(inp, action, response)
                self.protocol.respond(event.dest, m)

                with open(self.dbfile, "w+") as f:
                    f.writelines(json.dumps(self.db))
        # Generalize ownership API TODO
        elif cmd == "del" and self.protocol.config["owner"] == event.prefix[0]:
            _, message = event.msg.split(" ", 1)
            fact = self.db.pop(int(message))
            self.protocol.respond(event.dest, "Factoid '{}' deleted.".format(fact))
        elif cmd == "list":
            self.protocol.respond(event.dest, [i for i in enumerate(self.db)])
